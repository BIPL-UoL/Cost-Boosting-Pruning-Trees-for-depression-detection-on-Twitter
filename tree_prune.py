...
def pruning_order(self, max_to_prune=None):
    ...
    def _next_to_prune(tree, children=None):
        ...
        #Just check parent node of leaves so (Gt(parent_node) - Gt(leaves))/(leaves_number-1),
        # leaves_number ==2
...
#######################  BY XIAOHU ######################################
def prune_score(clf, X_valition, y_validation, max_n_leaves=10,sample_weight=None):
    from copy import deepcopy

    loc_scores = list()
    loc_clf = deepcopy(clf)
    for index,i in enumerate(range(max_n_leaves-1, 1, -1)):
        loc_clf.prune(n_leaves=i)
        clf_score = loc_clf.score(X_valition, y_validation,sample_weight=sample_weight)
        print('T{}_accuracy={}'.format(index,clf_score))
        loc_scores.append(clf_score)
    return loc_scores



def prune_path(clf, X, y, max_n_leaves=10, n_iterations=10,
                          test_size=0.1, random_state=None,sample_weight=None):
    ...
    kf = ShuffleSplit(n_splits=n_iterations, test_size=test_size, random_state=10)
    '''Test minimum number of estimator leaves'''
    leaves_list=[]
    estimator_list =[]

    for train,test in kf.split(X,y):
        min_leaf_estimator = clone(clf)
        if sample_weight is not None:

            min_leaf_estimator.fit(X[train], y[train], sample_weight=sample_weight[train])
        else:

            min_leaf_estimator.fit(X[train], y[train])
        leaves_list.append(get_n_leaves(min_leaf_estimator))
        estimator_list.append(min_leaf_estimator)
    min_leaf_num = np.min(leaves_list)-1

    for estimator in estimator_list:

        loc_scores = list()

        for i in range(min_leaf_num, 1, -1):
            ...
            if sample_weight is not None:
                loc_scores.append(estimator.score(X[test], y[test],sample_weight=sample_weight[test]))
            else:
                loc_scores.append(estimator.score(X[test], y[test]))
...
def prune_path_multiprocess(clf, X, y, max_n_leaves=10, n_iterations=10,
                          test_size=0.1, random_state=None,sample_weight=None,pruning_stride=1,pruning_start=0.9,pruning_end=0.1):
    """Cross validation of scores for different values of the decision tree.
    ...
    from ..base import clone
    from ..model_selection import ShuffleSplit
    from ..ensemble import boost_utils
    import gc
    ...
    #
    #
    # # Parameter range of leaves search
    # if min_leaf_num<10:
    #     leaves = np.arange(int(1 * min_leaf_num), 1, -1)
    # else:
    #     leaves = np.arange(int(1 * min_leaf_num), 1, -1)

    leaves = np.arange(int(min_leaf_num*pruning_start),int(min_leaf_num*pruning_end),-1*pruning_stride)
    #leaves = np.arange(min_leaf_num, 1, -1)
    Multi_cpu_class = boost_utils.Multicpu(cpu_num=n_iterations, thread_num=1
                               , ccp_alphas=leaves, random_state=random_state
                               , X=X, y=y, sample_weight=sample_weight)

    scores = boost_utils.multi_cpu(boost_utils.multi_cpu_train, split_index, Multi_cpu_class)
    del Multi_cpu_class
    # del kf
    gc.collect()
    return zip(*scores),leaves



def get_sample_depth(clf,X_matrix):
    """
    Get the depths of the decision tree

    """

    #node_ids = clf.apply(X_matrix)
    ...
    nodes_values = []
    nodes_samples = []
    for x_index in range(X_matrix.shape[0]):
        node_id = 0
        sample_depth=0
        while tree.children_left[node_id]!=-1: #and tree.children_right[node_id] != -1:
            sample_depth += 1
            if X_matrix[x_index,tree.feature[node_id]] <= tree.threshold[node_id]:
                node_id = tree.children_left[node_id]
            else:
                node_id = tree.children_right[node_id]
        nodes_id.append(node_id)
        nodes_depth.append(sample_depth)
        nodes_values.append(tree.init_error[node_id])
        nodes_samples.append(tree.n_samples[node_id])
    return np.array(nodes_id),np.array(nodes_depth),np.array(nodes_values),np.array(nodes_samples)
